...
class GameRoom {
  constructor(roomId, hostId, hostName, hostUserId = null, hostScoreBoard = 0, isPublic = true) {
    this.roomId   = roomId;
    ...
    this.roundNumber      = 0;
    this.lastSwap         = null; // { swapperName, first:{playerId,position}, second:{playerId,position} }
  ...
  addPlayer(socketId, name, isHost = false, userId = null, scoreBoard = 0) {
    if (this.players.length >= 8) return { error: 'Room is full' };
    if (this.phase !== 'lobby') return { error: 'Game already in progress' };
    this.players.push({
      id: socketId,
      userId,        // auth user ID — used to write stats to DB
      name,
      isHost,
      grid: [], hand: [],
      score: 0,
      scoreBoard,    // loaded from DB; persists across rounds in this room
    });
    return { ok: true };
  }
  ...
  // Allows restart from 'finished' — preserves scoreBoard.
  startGame() {
    ...
    this.roundNumber++;           // track which round we're on
    ...
    for (const p of this.players) {
      ...
      p.hand = [];
      // scoreBoard intentionally preserved
    }
  ...
  // Called before the player draws; they forfeit their turn.
  // Turn is advanced by the caller (index.js) via advanceTurnAndCheck.
  callKaaboo(playerId) {
    this.lastSwap = null;
  ...
  // Resolves the game when the Kaaboo caller's turn comes around again.
  endGame() {
    ...
    // Strictly lowest: caller must be less than ALL other players (no ties)
  ...
  // Keeps the top discard card in place; shuffles the rest back into the deck.
  // Returns true if the deck now has cards, false if there was nothing to reshuffle.
  _reshuffleDiscardIntoDeck() {
    if (this.deck.length > 0) return true;
    if (this.discardPile.length <= 1) return false; // only the top card (or empty) — nothing to reshuffle
  ...
  replaceGridCard(playerId, gridPosition) {
    const player = this.players.find((p) => p.id === playerId);
    if (!player) return { error: 'Player not found' };
  ...
  openPlaydownWindow(discardType, currentTurnPlayerId) {
  ...
  attemptPlayDown(attempterId, cardOwnerId, gridPosition) {
  ...
  openPowerDecisionWindow(playerId, cardRank) {
  ...
  powerPeek(playerId, targetPlayerId, gridPosition) {
  ...
  powerSwapSelect(playerId, targetPlayerId, gridPosition) {
    const pw = this.powerWindow;
    if (!pw || pw.phase !== 'action') return { error: 'No active power action' };
    if (pw.playerId !== playerId) return { error: 'Not your power' };
  ...
  getFinalScores() {
    return this.players
      .map((p) => ({ id: p.id, name: p.name, score: this.calculatePlayerScore(p.id) }))
  ...
  _resolveCardVisibility(ownerId, position, card, viewerId) {
    // Finished: all cards revealed to everyone
    if (this.phase === 'finished') return card;
    // Peek phase: player sees own bottom two
    if (this.phase === 'peek' && ownerId === viewerId && PEEK_POSITIONS.has(position)) {
      return card;
    }
    // Power peek reveal
  ...
  getPublicState() {
    return {
      ...
      giveCardWindow: this.giveCardWindow
        ? {
            endsAt: this.giveCardWindow.endsAt,
            giverId: this.giveCardWindow.giverId,
            receiverId: this.giveCardWindow.receiverId,
          }
        : null,
      powerWindow: this.powerWindow
        ? {
            ...
            peekedCards: this.powerWindow.peekedCards,   // positions only — card values still hidden
          }
        : null,
      lastSwap: this.lastSwap,
      // Final results — only populated when phase === 'finished'
...
module.exports = { GameRoom, getPowerInfo };
