import { useEffect, useState } from 'react';
import { Card } from './Card';
import type { Card as CardType } from '@/types/game';

export interface GiveFromSlot {
  rect: { top: number; left: number; width: number; height: number };
  card: CardType | null;
}
export interface GiveToSlot {
  rect: { top: number; left: number; width: number; height: number };
}

interface GiveCardAnimProps { from: GiveFromSlot; to: GiveToSlot; }

export function GiveCardAnim({ from, to }: GiveCardAnimProps) {
  const [flying, setFlying] = useState(false);

  useEffect(() => {
    let r1 = 0;
    let r2 = 0;
    r1 = requestAnimationFrame(() => { r2 = requestAnimationFrame(() => setFlying(true)); });
    return () => { cancelAnimationFrame(r1); cancelAnimationFrame(r2); };
  }, []);

  const dx = to.rect.left - from.rect.left;
  const dy = to.rect.top  - from.rect.top;

  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 499 }}>
      <div
        style={{
          position: 'fixed',
          left:   from.rect.left - (window.visualViewport?.offsetLeft ?? 0),
          top:    from.rect.top  - (window.visualViewport?.offsetTop  ?? 0),
          width:  from.rect.width  || 72,
          height: from.rect.height || 100,
          pointerEvents: 'none',
          transition: flying ? 'transform 0.75s cubic-bezier(0.4,0,0.2,1), opacity 0.75s ease' : 'none',
          transform: flying ? `translate(${dx}px, ${dy}px)` : 'translate(0,0)',
        }}
      >
        <Card card={from.card} faceDown={!from.card} />
      </div>
    </div>
  );
}

